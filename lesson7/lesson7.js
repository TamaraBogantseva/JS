...
var BOMB_CREATE = 9000; //Интервал между появлением бомб
var snake = []; // Сама змейка
var direction = 'y+'; // Направление движения змейки
var gameIsRunning = false; // Запущена ли игра
var snake_timer; // Таймер змейки
var bomb_timer; // Таймер бомб
...
function init() {
    ...
    */
    ...
    document.getElementById('snake-start').addEventListener('click', startGame); //тут есть баг, когда после проигрыша он продолжает начатую игру после нажатия кнопки старт 
...
function prepareGameField() {
    ...
    document.querySelector(".score").innerHTML = ("Ваш счет равен: ") + score; //добавление счета
...
function startGame() {
    ...
    bomb_timer = setInterval(createBomb, BOMB_CREATE);
}
...
function move() {
    ...
    if (new_unit == undefined) {
        if (direction == 'x-') {
            new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (19))[0]; //последнее значение столбца на игровом поле
        }
        else if (direction == 'x+') {
            new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (0))[0]; //первое значение столбца
        }
        else if (direction == 'y+') {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        console.log(score);
        document.querySelector(".score").innerHTML = ("Ваш счет равен: ") + score;
...
function createBomb() {
    ...
    while (!bombCreated) {
        ...
        //проверка на змейку и на ячейки с едой 
